...
static void usart_clk_init(void)
{
	...
	RCC_APB2PeriphClockCmd(DEBUG_GPIO_RCC, ENABLE);
}
static void usart_gpio_init(void)
{
	GPIO_InitTypeDef gpio_init_structure;
	//Config UART5: PC12 = TX, PD2 = RX
	gpio_init_structure.GPIO_Pin = DEBUG_TX;
	gpio_init_structure.GPIO_Mode = GPIO_Mode_AF_PP;
	gpio_init_structure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(DEBUG_GPIO, &gpio_init_structure);
	gpio_init_structure.GPIO_Pin = DEBUG_RX;
