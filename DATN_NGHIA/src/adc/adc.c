...
static void adc_module_init()
{
	...
	ADC_RegularChannelConfig(VOLTAGE_ADC, ADC_Channel_1, 1, ADC_SampleTime_55Cycles5);
	/* Enable ADC1 DMA */
	ADC_DMACmd(VOLTAGE_ADC, ENABLE);
	/* Enable ADC1 */
	ADC_Cmd(VOLTAGE_ADC, ENABLE);
	/* Enable ADC1 reset calibration register */
	ADC_ResetCalibration(VOLTAGE_ADC);
	/* Check the end of ADC1 reset calibration register */
	while(ADC_GetResetCalibrationStatus(VOLTAGE_ADC));
	/* Start ADC1 calibration */
	ADC_StartCalibration(VOLTAGE_ADC);
	/* Check the end of ADC1 calibration */
	while(ADC_GetCalibrationStatus(VOLTAGE_ADC));
	/* Start ADC1 Software Conversion */
	ADC_SoftwareStartConvCmd(VOLTAGE_ADC, ENABLE);
}
void adc_init()
{
	adc_clk_init();
...
void get_current_load(float *current){
	...
	adc_vol =(float)(adc_val/4096.0)*3.3;
	*current = ((adc_vol - 2.5)*1000.0)/66.0;
	return ;
}
