...
{
  LCD_CS_ON;
  LCD_DATA16_WRITE(Data); // @suppress("Field cannot be resolved")
  LCD_CS_OFF;
}
...
//-----------------------------------------------------------------------------
static void LCD_data16_write(uint16_t dat)
{
	LCD_DATA16_WRITE(dat);
}

uint16_t RGB888_to_RGB565(uint32_t RGBcode)
{
	uint8_t R,G,B;
	B = RGBcode & (0x0000FF);
	G = RGBcode & (0x00FF00) >> 8;
	R = RGBcode >> 16;
...
void LCD_IO_WriteCmd8MultipleData16_Monochrome(uint8_t Cmd, uint16_t *pData, uint32_t Size)
{
  ...
  while(Size--)
  {
	  ...
	  for(int i=0;i<8;i++)
	  {
		  if (temp&(0x80>>i)) LCD_data16_write(0);
		  else
		  {
			  LCD_data16_write(0xffff);
		  }
	  }
  }
  LCD_CS_OFF;
...
//-----------------------------------------------------------------------------
void LCD_IO_WriteCmd8MultipleData16_Grayscale4(uint8_t Cmd, uint16_t *pData, uint32_t Size)
{
  LCD_CS_ON;
  LCD_CMD8_WRITE(Cmd);
  uint16_t temp;
  ...
  {
	  temp = *pData;
	  pixel1 =RGB888_to_RGB565((temp&0xF0)*1118481);
	  LCD_data16_write(pixel1);
	  pixel2 =RGB888_to_RGB565((temp&0x0f)*1118481);
	  LCD_data16_write(pixel2);
	  pData ++;
  }
  LCD_CS_OFF;
}
